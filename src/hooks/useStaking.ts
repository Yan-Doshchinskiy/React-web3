...
import { ITokensAPI } from "./useTokens";
...
const initialTokenState: IStaking = {
  address: '',
  totalStaked: '0',
  userStaked: '0',
  claimable: '0',
...
const useStaking = ({ tokensAPI }: IProps): IStakingAPI => {
  const { stakingToken, extendStakingToken, extendRewardToken, extendTokensData } = tokensAPI;
  const isWalletConnected = useSelector(web3Getters[WEB3_GETTERS.GET_WALLET_IS_CONNECTED]);

  const [staking, setStaking] = useState<IStaking>({ ...initialTokenState, address: stakingContract });
  const [transactions, setTransactions] = useState<Array<ITransactionRow>>([]);
