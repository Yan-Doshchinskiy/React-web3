...
import { IStakingAPI } from "../../hooks/useStaking";
...
interface IProps {
  ...
  stakingAPI: IStakingAPI
}

export const StakingPanel = ({ tokensAPI, stakingAPI }: IProps) => {
  const [isApprove, setIsApprove] = useState(false);
  const isWalletConnected = useSelector(web3Getters[WEB3_GETTERS.GET_WALLET_IS_CONNECTED]);
  const { stakingToken, mintStakingToken, approve } = tokensAPI;
  const { staking, stake, unstake } = stakingAPI;
